...
void parse_segment_track_entry(struct matroska_ctx* mkv_ctx) {
    ...
    if (track_type == MATROSKA_TRACK_TYPE_SUBTITLE)
    {
        struct matroska_sub_track** temp;
...
char* generate_filename_from_track(struct matroska_ctx* mkv_ctx, struct matroska_sub_track* track)
{
    ...
    if (track->lang_index == 0)
        sprintf(buf, "%s_%s.%s", mkv_ctx->ext_stripped_filename, track->lang, matroska_track_text_subtitle_id_extensions[track->codec_id]);
    else
        sprintf(buf, "%s_%s_" LLD ".%s", mkv_ctx->ext_stripped_filename, track->lang, track->lang_index,
...
int matroska_loop(struct lib_ccx_ctx *ctx)
{
    ...
    mkv_ctx->ext_stripped_filename = filename_without_ext(mkv_ctx->filename );
